...
#SBATCH -J OxCharCombo10_betterFunctionalCf1
...
sed -e "s///g" Combo10_betterFunctional_Cf1TEMPLATE.inp
...
do
    ...
    do
        ...
        # Name of files
        infile="Combo10_betterFunctional_Cf1tc_${D1[j]}_${D2[j]}_ck${CKVOLTAGE[i]}.inp"
        outfile="Combo10_betterFunctional_Cf1tc_${D1[j]}_${D2[j]}_ck${CKVOLTAGE[i]}.out"

        # Copy pointcharge file for debug purpose
        pcfile="Combo10_betterFunctional_Cf1tc_${D1[j]}_${D2[j]}_ck${CKVOLTAGE[i]}.pc"
        ...
        if [[ -f "$outfile" ]]; then
            ...
            # Create input file
            cp Combo10_betterFunctional_Cf1TEMPLATE.inp $infile
