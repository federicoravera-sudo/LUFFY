...
do
    ...
    do
        ...
        # Name of files
        infile="Combo10_Cf4tc_${D1[j]}_${D2[j]}_ck${CKVOLTAGE[i]}.inp"
        outfile="Combo10_Cf4tc_${D1[j]}_${D2[j]}_ck${CKVOLTAGE[i]}.out"

        # Copy pointcharge file for debug purpose
        pcfile="Combo10_Cf4tc_${D1[j]}_${D2[j]}_ck${CKVOLTAGE[i]}.pc"
        ...
        if [[ -f "$outfile" ]]; then
            ...
            # Create input file
            cp Combo10_Cf4TEMPLATE.inp $infile
